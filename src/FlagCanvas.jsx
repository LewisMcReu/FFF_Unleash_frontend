...
function drawLayerOnCanvas(context, layer) {
    ...
    if (layer['@type'] === 'basic') {
        ...
        if (layer['type'] === 'FIELD') {
            ...
            context.closePath();
            context.fill();
        } else if (layer['type'] === 'PALE') {
            context.fillRect(width * .35, 0, width * .3, height);
        } else if (layer['type'] === 'FESS') {
            context.fillRect(0, height * .35, width, height * .3);
        } else if (layer['type'] === 'BEND') {
            const path = new Path2D();
            path.rect(0, -height * .3 / 2, width * 1.2, height / 3);
            ...
            context.fillRect(width * .7, 0, width * .3, height);
            ...
            const crossWidth = height / 6;
            const crossLength = crossWidth * 4;
            context.fillRect(crossWidth * 4, crossWidth, crossWidth, crossLength);
            context.fillRect(crossWidth * 2.5, crossWidth * 2.5, crossLength, crossWidth);
        } else if (layer['type'] === 'SYMMETRIC_CROSS') {
            const crossWidth = height / 6;
            context.fillRect(crossWidth * 4, 0, crossWidth, height);
            context.fillRect(0, crossWidth * 2.5, width, crossWidth);
        } else if (layer['type'] === 'NORDIC_CROSS') {
            const crossWidth = height / 6;
            context.fillRect(width / 3, 0, crossWidth, height);
            context.fillRect(0, crossWidth * 2.5, width, crossWidth);
